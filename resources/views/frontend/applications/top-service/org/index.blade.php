        ...
        @if($oldApplication)
            <div class="row p-3 justify-content-center">
                ...
                <table class="col-md-8 table table-bordered mt-3">
                    ...
                    <tr>
                        <th>সার্ভিস</th>
                        <td>{{ $oldApplication->incomeable->name }}</td>
                    </tr>
                    <tr>
                        <th>প্যাকেজ</th>
                ...
                <div class="col-2 text-center">
                    <a href="{{ route('frontend.applications.organization-top-service.edit', $oldApplication->id) }}">
                        <button role="button" class="btn btn-info btn-block">এডিট</button>
                    </a>
                </div>
            </div>
        @else
            <form action="{{ route('frontend.applications.organization-top-service.store') }}" class="row p-3 justify-content-center" method="post">
                {{ csrf_field() }}
                <div class="col-12">
                        ...
                        <div class="col-md-8">
                            <select name="service" id="service-select" class="form-control{{ $errors->has('service') ? ' is-invalid' : '' }}">
                                ...
                                @foreach($services as $service)
                                    ...
                                    @if(old('service') == $service->id || ( ! old('service') && request()->get('service') == $service->id )){{ 'selected' }}@endif>
                                        {{ $service->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-8">
                            <select name="package" id="package-select" class="form-control{{ $errors->has('package') ? ' is-invalid' : '' }}">
                                ...
                                @foreach($packages as $package)
                                    ...
                                    @if(old('package') == $package->id){{ 'selected' }}@endif>
                        ...
                        <div class="col-md-8">
                            <select name="payment-method" id="method-select" class="form-control{{ $errors->has('payment-method') ? ' is-invalid' : '' }}">
                                ...
                                @foreach($paymentMethods as $paymentMethod)
                                    ...
                                    @if(old('payment-method') == $paymentMethod->id){{ 'selected' }}@endif>
                                        {{ $paymentMethod->name }}
                                    </option>
                                @endforeach
                            </select>
